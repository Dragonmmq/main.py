...
async def handle(msg: types.Message):
    ...
    if user.id in last_msg and now - last_msg.get(user.id, 0) < SPAM_DELAY:
    ...
    cursor.execute("INSERT INTO messages (user_id, text, media_type, file_id) VALUES (?, ?, ?, ?)", (user.id, db_text, media_type, file_id))
    conn.commit()
