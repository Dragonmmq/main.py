...
ADMINS = [8394162540]           # ← твой Telegram ID
CHANNEL_ID = -1003285603970     # ← ID канала
...
# ========= ОСНОВНАЯ ЛОГИКА (с голосовыми и кружочками) =========
...
async def handle(msg: types.Message):
    ...
    if msg.photo:
        ...
        extra_info = msg.sticker.emoji or ""
    elif msg.voice:
        media_type = "voice"
        file_id = msg.voice.file_id
        extra_info = f"({msg.voice.duration} сек)" if msg.voice.duration else ""
    elif msg.video_note:
        media_type = "video_note"
        file_id = msg.video_note.file_id
        extra_info = f"({msg.video_note.duration} сек)" if msg.video_note.duration else ""
    ...
    if get_forwarding():
        try:
            if text:
                ...
                await bot.send_message(CHANNEL_ID, f"<blockquote>🎤 Анонимное голосовое {extra_info}</blockquote>")

            elif media_type == "video_note":
                await bot.send_video_note(CHANNEL_ID, file_id)
                await bot.send_message(CHANNEL_ID, f"<blockquote>🔄 Анонимный кружочек {extra_info}</blockquote>")
    ...
    elif media_type in ["sticker", "voice", "video_note"]:
        emoji = "🎨" if media_type == "sticker" else "🎤" if media_type == "voice" else "🔄"
        admin_text += f"{emoji} {media_type.replace('_', ' ').title()} {extra_info}"
    ...
    for admin in ADMINS:
        try:
            # Текст + стикер + голосовое + кружочек
    ...
    # Запись в базу
    db_text = text if text else f"{media_type} {extra_info}".strip()
    cursor.execute(
